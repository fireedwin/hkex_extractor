...
本檔案裡的每一項都會**改變萃取結果**,所以動了它就該讓既有結果失效、
全部重新分析(增量處理會自動偵測)。這是刻意的:補了一個科目別名,
舊的擷取結果就是過期的。

反過來,「不影響萃取結果」的操作設定(下載哪一類文件、錯誤紀錄怎麼寫)
放在 ops_config.py,改它不會觸發全量重跑。新增設定前先問一次:
**改了它,同一份 PDF 產出的 Excel 會不會不一樣?**
"""
...
# --------------------------------------------------------------------------
# 4. 操作設定 —— 已搬到 ops_config.py
#
#    下載文件類型(DOC_TYPES)與錯誤紀錄規則(ERROR_*)不影響任何一份
#    PDF 的萃取結果,卻曾經因為跟科目別名放在同一個檔案裡,害得
#    「調一次錯誤訊息措辭 → 幾百份年報全部重跑」。所以搬出去,
#    並在 incremental.py 排除,改它不再觸發全量重分析。
#
#    這裡 re-export 是為了向下相容:既有的 config.DOC_TYPES、
#    config.ERROR_PATTERNS 等寫法照常可用,不必改其他檔案。
#    這一行本身不會變動,所以 ops_config.py 怎麼改都不影響
#    config.py 的雜湊值 —— 隔離才成立。
#
#    ⚠ 新增設定前先問:「改了它,同一份 PDF 的 Excel 會不一樣嗎?」
#      會 → 留在本檔;不會 → 放 ops_config.py。
# --------------------------------------------------------------------------
from ops_config import (  # noqa: F401,E402
    DOC_TYPES,
    DEFAULT_DOC_TYPE,
    ERROR_DIR,
    ERROR_SEVERITY_ORDER,
